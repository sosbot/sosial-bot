...
func webhookHandler( /*c *gin.Context*/ w http.ResponseWriter, r *http.Request) {
	...
	if update.CallbackQuery != nil {
		...
		logger(update.Message.Chat.ID, "Top message chat id  "+fmt.Sprintf("%f", update.CallbackQuery.Message.Chat.ID), LogAppInfo)
		logger(update.Message.Chat.ID, "Top message  id  "+fmt.Sprintf("%f", update.CallbackQuery.Message.MessageID), LogAppInfo)
...
func execQuestionsAnswer(update *tgbotapi.Update, QuestionTypeName string, chat_id int64, currentState int, answer string) {
	...
	case 3:
		logger(update.Message.Chat.ID, "inside message chat id  "+fmt.Sprintf("%f", update.CallbackQuery.Message.Chat.ID), LogAppInfo)
		logger(update.Message.Chat.ID, "inside message  id  "+fmt.Sprintf("%f", update.CallbackQuery.Message.MessageID), LogAppInfo)
