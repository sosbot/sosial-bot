...
func webhookHandler( /*c *gin.Context*/ w http.ResponseWriter, r *http.Request) {
	...
	// var id int
	err = db.QueryRow("insert into public.messages(text,sent,sentby,tel_chat_id,tel_message_id) values($1,$2,$3,$4,$5);", update.Message.Text, time.Now(), update.Message.From.ID, update.Message.Chat.ID, update.Message.MessageID).Scan()
	if err != nil {
		log.Println(err)
		return
	}
